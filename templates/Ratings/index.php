		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Rating'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('bookings_id') ?></th>
                    <th><?= $this->Paginator->sort('vehicles_id') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= ($rating->booking->nric)?></td>
                    <td><?= ($rating->vehicle->model) ?></td>
                    <td><?= h($rating->rate) ?></td>
                    <td><?= h($rating->comment) ?></td>
                    <td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $rating->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $rating->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Ratings: "{0}"?', $rating->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_ratings_active; ?></h3>
					<p>Active Ratings</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_ratings_archived; ?></h3>
					<p>Archived Ratings</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Ratings (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Ratings by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Ratings(s)',
			data: [<?= json_encode($total_ratings_active); ?>, <?= json_encode($total_ratings_disabled); ?>, <?= json_encode($total_ratings_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Ratings','action' => 'index']]); ?>
				<fieldset>
					<div class="mb-1"><?php echo $this->Form->control('id',['required' => false]); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('rate', ['required' => false]); ?></div>
				</fieldset>
		<div class="text-end">
			<?php 
				if (!empty($_isSearch)) {
