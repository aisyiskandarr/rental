		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Vehicle'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
            ...
            <tr>
                ...
                <th><?= $this->Paginator->sort('rate_perhour') ?></th>
        ...
        <tbody>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= h($vehicle->manufacturer) ?></td>
                    <td><?= h($vehicle->registration_no) ?></td>
                    <td><?= h($vehicle->color) ?></td>
                    <?php
						$type = in_array($vehicle->model, ['EX5', 'LC']) ? 'motorcycle' : 'car';
						?>
						<td><?= h($type) ?></td>
                    <td class="text-center"><?= h($vehicle->rate_perhour) ?></td>
                    <td class="text-center"><?php if ($vehicle->image == NULL) {
					...
					<?php if ($vehicle->status ==0) {
							echo '<span class= "badge bg-warning">Reserved</span>';
						}elseif  ($vehicle->status ==1) {
						echo '<span class= "badge bg-success">Available</span>';
					}elseif  ($vehicle->status ==2) {
						echo '<span class= "badge bg-danger">Not Available</span>';
					}else 
						echo '<span class= "badge bg-primary">In Services</span>';
						?></td>

					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $vehicle->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $vehicle->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Vehicles: "{0}"?', $vehicle->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_vehicles_active; ?></h3>
					<p>Active Vehicles</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_vehicles_archived; ?></h3>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Vehicles (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Vehicles by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Vehicles(s)',
			data: [<?= json_encode($total_vehicles_active); ?>, <?= json_encode($total_vehicles_disabled); ?>, <?= json_encode($total_vehicles_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Vehicles','action' => 'index']]); ?>
				<fieldset>
					...
					echo $this->Form->select('status', $options, ['multiple' => 'checkbox', 'class' => 'form-check-input']); ?></div>
				</fieldset>
		<div class="text-end">
			<?php 
				if (!empty($_isSearch)) {
